...
<ul class="nav nav-tabs">
    <li ${empty param['search.show_eq'] ? 'class="active"' : ''}>
        <a href="${ctx}/bussiness/tree/tree?async=${not empty param.async and param.async eq true}">
    ...
    <li ${not empty param['search.show_eq'] ? 'class="active"' : ''}>
        <a href="${ctx}/bussiness/tree/tree?async=${not empty param.async and param.async eq true}&search.show_eq=true">
            <i class="icon-table"></i>
            显示的
        </a>
    </li>
            ...
            setting : {
                callback : {
                    onClick: function(event, treeId, treeNode, clickFlag) {
                        var url = '${ctx}/bussiness/tree/' + treeNode.id + '/update';
