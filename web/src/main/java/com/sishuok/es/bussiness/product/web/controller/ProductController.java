 ...
 */
@Controller
@RequestMapping(value = "/bussiness/product/product")
public class ProductController extends BaseCRUDController<Product, Long> {
    ...
    public String create2(Model model, @Valid @ModelAttribute("m") Product product, BindingResult result, RedirectAttributes redirectAttributes) {
    ...
    public String find( Model model,@CurrentUser User user) {

        if (permissionList != null) {
            this.permissionList.assertHasViewPermission();
        }
        model.addAttribute("user", user);
        return viewName("first_editForm");
    }
    @RequestMapping(value = "/listFind", method = RequestMethod.POST)
    public String listFind(Searchable searchable, Model model) {

        if (permissionList != null) {
            this.permissionList.assertHasViewPermission();
        }

        model.addAttribute("list", baseService.findAllWithNoPageNoSort(searchable));
       
        return viewName("listFind");
    }
   
    @RequestMapping(value = "create3", method = RequestMethod.POST)
    public String create3(
            Model model, @Valid @ModelAttribute("m") Product product, BindingResult result,
